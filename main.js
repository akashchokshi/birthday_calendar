...
function populateNamesAndBirthdaysDayWise() {
  namesAndBirthdays = JSON.parse(
    document.getElementById("namesAndBirthdays").value
  );
  ...
  namesAndBirthdays = namesAndBirthdays.filter(
    (person) =>
      parseInt(new Date(person["birthday"]).getFullYear(), 10) ===
      currentBirthdayYear
  );
  // sort datewise in ascending order
  namesAndBirthdays = namesAndBirthdays.sort((a, b) =>
...
function sortBirthdaysDayWise(name, dayOfWeek) {
  ...
  birthdaysDayWise[dayOfWeek]["birthdays"].push(
  ...
  for (const day in birthdaysDayWise) {
    if (birthdaysDayWise.hasOwnProperty(day)) {
      ...
      if (personsInitialsArrayLength > 0) {
        ...
        const heightWidthForBirthdays =
          100 / (Math.floor(Math.sqrt(personsInitialsArrayLength)) + delta);
        let colorIndex = 0;
        for (let index = 0; index < personsInitialsArrayLength; index++) {
          const initials = personsInitialsArray[index];
          const birthday = createDivForBirthdays(
            initials,
            heightWidthForBirthdays,
            colorIndex
          );
          colorIndex++;
          // reset colorIndex
